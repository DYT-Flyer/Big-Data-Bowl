...
def getolinesix(play):
    oline = play.loc[
            (play.loc[:, "Position"] == "T") | (play.loc[:, "Position"] == "G") | (play.loc[:, "Position"] == "C") | (
                        play.loc[:, "Position"] == "OT") | (play.loc[:, "Position"] == "OG"), :]
    oline = oline.sort_values(by='Y')
...
for i in range(0, uniqueplays.shape[0]):
    ...
    fpos = encodefpos(play, uniqueteams)
    play, ha, ydline, bc, leftright, bcdir = changedirection(play, pos, fpos)
    olnum, olnumeng, numdefbf, boxsafety, bchole, oline, holesize, holeslope, push = hmengagedextra(play, ha, ydline,
                                                                                                    bc)
...
train_holefeatures = np.concatenate(
    (olnums, olnumseng, numdefsbf, boxsafeties, bcshole, leftsrights, bcdirs, holes_size, holes_slope, pushes, ydlines),
    axis=1)
